...
int32_t main()
{
    ...
    while(t--){
        ...
        for(int i=1 ;i<=n ;i++){
            ...
            if( st.find(b) != st.end() ){     // if inversion were found
                a1 += temp;
                a2 = b+a2;
                st.erase(temp);
                
                auto pos = palin.find(temp);
                if(pos != palin.end()) palin.erase(pos);
            }
            else if( b == temp){         // is palindrome
                st.insert( temp );
                palin.insert(temp);
            }
        ...
        if(palin.size() == 0){
            ...
            int mx=0;
            string vache = *palin.begin();

            cout << a1.size() + vache.size() + a2.size() << endl;
            cout << a1 + vache  + a2 << endl;
