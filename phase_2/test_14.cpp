...
int32_t main()
{
    ...
    while(t--){
        ...
        if(t2 > 0) for(int i=0 ;i<t1 ;i++) ans+='L';
        if(t1 > 0) for(int i=0 ;i<t2 ;i++) ans+='D';
        if(t2 > 0) for(int i=0 ;i<t1 ;i++) ans+='R';
        if(t1 > 0) for(int i=0 ;i<t2 ;i++) ans+='U';

        if(t2 == 0 and t1!=0) ans = "LR";
        if(t1 == 0 and t2!=0) ans = "UD";
