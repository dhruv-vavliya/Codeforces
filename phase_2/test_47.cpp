...
public:
    int sumOfFlooredPairs(vector<int>& nums) {
        unsigned lli n = nums.size();
        unsigned lli ans=0;

        sort( all(nums) );
        for(int i=0 ;i<n ;i++){     // take one y one value
            unsigned lli pos=0;

            for(lli k=nums[i] ;k<=nums[n-1] ;k+=nums[i] ){
                pos = lower_bound( nums.begin()+pos ,nums.end() ,k) - nums.begin();
                ans += n-pos;
                ans%=mod;
            }
        }
