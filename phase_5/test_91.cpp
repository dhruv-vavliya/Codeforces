...
void solve(){
    ...
    for(auto val : mp){
        int s = val.second;
        if( val.first != 1 and st.find(val.first-1) == st.end() ) st.insert(val.first-1) ,s--;
        if( s and st.find(val.first) == st.end() ) st.insert(val.first) ,s--;
        if( s ) st.insert(val.first+1);  
    }
    cout << st.size() << endl;
}
