...
int32_t main()
{
    ...
    while(t--){
        ...
        if(f2 == 0) cout << 0 << endl;                        // sorted
        else if(f1 == 0) cout << 1 << endl;                   // all have different pos
        else{
            int i=1;
            while(i<=n and a[i] == i) i+=1;
            int j=n;
            while(j>=1 and a[j] == j) j-=1;

            // cout  << i << ' ' << j << ' ' << n-j+i-1 << endl;

            if((n-j + i-1) == f1) cout << 1 << endl;          // left - right are set
            else cout << 2 << endl;                           // inside unset
        }
